...
const (
	ProxyListHideIPMe    = "https://raw.githubusercontent.com/zloi-user/hideip.me/refs/heads/main/http.txt"
	ProxyListProxlify    = "https://raw.githubusercontent.com/proxifly/free-proxy-list/main/proxies/protocols/http/data.txt" // nolint: revive
	ProxyListProxyMaster = "https://github.com/MuRongPIG/Proxy-Master/blob/main/http_checked.txt"
	ProxyListRoosterKid  = "https://raw.githubusercontent.com/roosterkid/openproxylist/main/HTTPS_RAW.txt"
	ProxyListTheSpeedX   = "https://raw.githubusercontent.com/TheSpeedX/PROXY-List/refs/heads/master/http.txt"
...
var (
	...
	AllProxyLists = []string{
		ProxyListHideIPMe,
		ProxyListProxlify,
		ProxyListProxyMaster,
		ProxyListRoosterKid,
		ProxyListTheSpeedX,
	}
)
...
func (c *ProxyClientConfig) applyDefaults() {
	if len(c.ProxyLists) == 0 {
		c.ProxyLists = AllProxyLists
	}
	if c.TestURL == "" {
...
func newProxyTransport(config *ProxyClientConfig) (http.RoundTripper, error) {
	proxyLists, err := downloadProxyLists(config.ProxyLists)
	if err != nil {
